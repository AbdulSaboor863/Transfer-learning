...
def process_data(data, smiles_col=SMILES_COL, n_features=24, is_target=False, pca=None,
    ...
    fingerprint_df.to_csv(os.path.join(output_dir, f"{'target' if is_target else 'source'}_fingerprints.csv"),
                          index=False)
...
if __name__ == '__main__':
    ...
    X_tgt_df, conversion_tgt, selectivity_tgt, _ = process_data(target_df, is_target=True, pca=pca,
    ...
    X_src_scaled = np.hstack(
        [X_src_uniform_scaled, X_src_other_scaled]) if X_src_other_scaled.size else X_src_uniform_scaled
    X_tgt_scaled = np.hstack(
        [X_tgt_uniform_scaled, X_tgt_other_scaled]) if X_tgt_other_scaled.size else X_tgt_uniform_scaled
    ...
    # MODIFIED t-SNE visualization before VAE - exclude specified features
    print("\nCreating t-SNE visualization before VAE (excluding specified features)...")

    # Get indices of features to exclude
    exclude_features = ['Boiling Point ', 'Melting Point', 'Density', 'Molecular weight']
    exclude_indices = [i for i, col in enumerate(feature_cols) if col in exclude_features]

    # Create filtered feature sets
    X_src_filtered = np.delete(X_src_scaled, exclude_indices, axis=1)
    X_tgt_filtered = np.delete(X_tgt_scaled, exclude_indices, axis=1)

    # Combine for t-SNE
    combined_features_before = np.vstack([X_src_filtered, X_tgt_filtered])
    tsne_before = TSNE(n_components=2, random_state=42)
    tsne_embedding_before = tsne_before.fit_transform(combined_features_before)

    # --- ADDED: Save CSV for t-SNE before VAE (excluding specified features) ---
    df_tsne_before = pd.DataFrame({
        'Dim1': tsne_embedding_before[:, 0],
        'Dim2': tsne_embedding_before[:, 1],
        'Domain': ['Source'] * len(X_src_filtered) + ['Target'] * len(X_tgt_filtered)
    })
    df_tsne_before.to_csv(os.path.join(output_dir, 'tsne_before_vae_filtered.csv'), index=False)
    print(f"Saved tsne_before_vae_filtered.csv in {output_dir}")
    # ------------------------------------------------------------------------

    plt.figure(figsize=(10, 8))
    plt.scatter(tsne_embedding_before[:len(X_src_filtered), 0],
                tsne_embedding_before[:len(X_src_filtered), 1],
                label='Source', alpha=0.7)
    plt.scatter(tsne_embedding_before[len(X_src_filtered):, 0],
                tsne_embedding_before[len(X_src_filtered):, 1],
                label='Target', alpha=0.7)
    plt.title('t-SNE Visualization (Excluding Physical Properties)')
    plt.xlabel('t-SNE Dimension 1')
    plt.ylabel('t-SNE Dimension 2')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'tsne_before_vae_filtered.png'))
    plt.close()
    print(f"Saved tsne_before_vae_filtered.png in {output_dir}")
    ...
    # Get full latent space (32D) for both source and target
    Z_src_full = vae.encode(torch.tensor(X_src_scaled, dtype=torch.float32))[0].detach().numpy()
    Z_tgt_full = vae.encode(torch.tensor(X_tgt_scaled, dtype=torch.float32))[0].detach().numpy()
    print(f"Full latent space shape for source: {Z_src_full.shape}")
    print(f"Full latent space shape for target: {Z_tgt_full.shape}")

    # Apply PCA to the VAE latent space (10D)
    Z_tgt = Z_tgt_full  # This is the full 32D latent space
    ...
    # MODIFIED t-SNE visualization after VAE - exclude specified features (using PCA-reduced latent space)
    print("\nCreating t-SNE visualization after VAE (excluding specified features, using PCA-reduced latent space)...")

    # For source data: use original scaled features (filtered)
    tsne_src = TSNE(n_components=2, random_state=42).fit_transform(X_src_filtered)

    # For target data: use latent+original features (filtered)
    # First remove the same features from the combined features
    X_tgt_combined = np.hstack([Z_tgt_reduced, X_tgt_scaled])
    # Calculate how many latent features we have (10)
    latent_dim = Z_tgt_reduced.shape[1]
    # The physical properties are in the original features part (after latent features)
    # So we need to adjust the indices for exclusion
    adjusted_exclude_indices = [i - latent_dim for i in exclude_indices if i >= latent_dim]
    X_tgt_combined_filtered = np.delete(X_tgt_combined, [i + latent_dim for i in exclude_indices if
                                                         i + latent_dim < X_tgt_combined.shape[1]], axis=1)

    tsne_tgt = TSNE(n_components=2, random_state=36).fit_transform(X_tgt_combined_filtered)

    # --- ADDED: Save CSV for t-SNE after VAE (PCA-reduced latent + filtered features) ---
    df_tsne_after_filtered = pd.DataFrame(
        np.vstack([tsne_src, tsne_tgt]),
        columns=['Dim1', 'Dim2']
    )
    df_tsne_after_filtered['Domain'] = ['Source'] * tsne_src.shape[0] + ['Target'] * tsne_tgt.shape[0]
    df_tsne_after_filtered.to_csv(os.path.join(output_dir, 'tsne_after_vae_filtered.csv'), index=False)
    print(f"Saved tsne_after_vae_filtered.csv in {output_dir}")
    # ------------------------------------------------------------------------

    plt.figure(figsize=(10, 8))
    plt.scatter(tsne_src[:, 0], tsne_src[:, 1],
                label='Source (Original Features)', alpha=0.7)
    plt.scatter(tsne_tgt[:, 0], tsne_tgt[:, 1],
                label='Target (With Latent Features)', alpha=0.7)
    plt.title('t-SNE Visualization After VAE (Excluding Physical Properties, PCA-reduced latent)')
    plt.xlabel('t-SNE Dimension 1')
    plt.ylabel('t-SNE Dimension 2')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'tsne_after_vae_filtered.png'))
    plt.close()
    print(f"Saved tsne_after_vae_filtered.png in {output_dir}")

    # NEW: t-SNE visualization after VAE using full latent space (32D) before PCA
    print("\nCreating t-SNE visualization after VAE (using full 32D latent space)...")

    # For target data: use full latent space + original features (filtered)
    X_tgt_combined_full = np.hstack([Z_tgt_full, X_tgt_scaled])
    # Remove the same physical properties from the original features part
    X_tgt_combined_full_filtered = np.delete(X_tgt_combined_full,
                                             [i + 32 for i in exclude_indices if i + 32 < X_tgt_combined_full.shape[1]],
                                             axis=1)

    tsne_tgt_full = TSNE(n_components=2, random_state=36).fit_transform(X_tgt_combined_full_filtered)

    # --- ADDED: Save CSV for t-SNE after VAE (full latent + filtered features) ---
    df_tsne_after_full = pd.DataFrame(
        np.vstack([tsne_src, tsne_tgt_full]),
        columns=['Dim1', 'Dim2']
    )
    df_tsne_after_full['Domain'] = ['Source'] * tsne_src.shape[0] + ['Target'] * tsne_tgt_full.shape[0]
    df_tsne_after_full.to_csv(os.path.join(output_dir, 'tsne_after_vae_full_latent.csv'), index=False)
    print(f"Saved tsne_after_vae_full_latent.csv in {output_dir}")
    # ------------------------------------------------------------------------

    plt.figure(figsize=(10, 8))
    plt.scatter(tsne_src[:, 0], tsne_src[:, 1],
                label='Source (Original Features)', alpha=0.7)
    plt.scatter(tsne_tgt_full[:, 0], tsne_tgt_full[:, 1],
                label='Target (With Full Latent Features)', alpha=0.7)
    plt.title('t-SNE Visualization After VAE (Using Full 32D Latent Space)')
    plt.xlabel('t-SNE Dimension 1')
    plt.ylabel('t-SNE Dimension 2')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'tsne_after_vae_full_latent.png'))
    plt.close()
    print(f"Saved tsne_after_vae_full_latent.png in {output_dir}")

    # NEW: 4th t-SNE visualization - Compare Z_src_full vs Z_tgt_full (latent space only)
    print("\nCreating 4th t-SNE visualization: Z_src_full vs Z_tgt_full (latent space only)...")

    # Combine source and target latent spaces
    combined_latent = np.vstack([Z_src_full, Z_tgt_full])

    # Apply t-SNE to the combined latent space
    tsne_latent = TSNE(n_components=2, random_state=25, learning_rate=15, max_iter=500, perplexity=1)
    tsne_embedding_latent = tsne_latent.fit_transform(combined_latent)

    # Split back into source and target
    tsne_src_latent = tsne_embedding_latent[:len(Z_src_full)]
    tsne_tgt_latent = tsne_embedding_latent[len(Z_src_full):]

    # Save CSV for t-SNE of latent space only
    df_tsne_latent = pd.DataFrame({
        'Dim1': tsne_embedding_latent[:, 0],
        'Dim2': tsne_embedding_latent[:, 1],
        'Domain': ['Source_Latent'] * len(Z_src_full) + ['Target_Latent'] * len(Z_tgt_full)
    })
    df_tsne_latent.to_csv(os.path.join(output_dir, 'tsne_latent_space_only.csv'), index=False)
    print(f"Saved tsne_latent_space_only.csv in {output_dir}")

    # Create the plot
    plt.figure(figsize=(10, 8))
    plt.scatter(tsne_src_latent[:, 0], tsne_src_latent[:, 1],
                label='Source Latent Space', alpha=0.7, color='blue')
    plt.scatter(tsne_tgt_latent[:, 0], tsne_tgt_latent[:, 1],
                label='Target Latent Space', alpha=0.7, color='orange')
    plt.title('t-SNE Visualization of VAE Latent Space (Z_src_full vs Z_tgt_full)')
    plt.xlabel('t-SNE Dimension 1')
    plt.ylabel('t-SNE Dimension 2')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'tsne_latent_space_only.png'))
    plt.close()
    print(f"Saved tsne_latent_space_only.png in {output_dir}")

    # Save combined data
    latent_cols = [f'latent_pca_{i}' for i in range(Z_tgt_reduced.shape[1])]
    combined_df = pd.DataFrame(
        np.hstack(
            [Z_tgt_reduced, X_tgt_scaled, conversion_tgt_scaled.reshape(-1, 1), selectivity_tgt_scaled.reshape(-1, 1)]),
